...
export function fixImports(files: string[], rootOptions: RootOption[]) {
  for (const file of files) {
    ...
    // deep copy
    const { newContents, originalContents } = createContentsFromImports({
      file,
      imports,
      rootOptions,
    });
...
interface CreateContentsFromImports {
  file: string;
  imports: [string, string][];
  rootOptions: RootOption[];
}

function createContentsFromImports({
  file,
  imports,
  rootOptions,
}: CreateContentsFromImports) {
